...
class KnotSettings(PropertyGroup):
    ...
    knot_file = StringProperty(
        name="Knot",
        description="Path to ASCII knot text file",
        subtype = "FILE_PATH"
        )
...
def add_knot(self, context, knot_string, z_scale, bias, scale, knot_name):
    ...
    mesh = bpy.data.meshes.new(name=knot_name)
...
class KnotOperator(bpy.types.Operator,AddObjectHelper):
    ...
    def execute(self, context):
        ...
        with open(knottool.knot_file, 'r') as file:
            knot = file.read()
            
        try:
            add_knot(self, context, knot, knottool.z_depth, knottool.z_bias, knottool.scale, bpy.path.display_name_from_filepath(knottool.knot_file))
...
class OBJECT_PT_my_panel(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop_search(context.scene.knot_tool, 'knot_file', bpy.data, 'texts', text="Knot")               
